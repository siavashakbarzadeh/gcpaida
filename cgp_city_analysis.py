"""
CGP (Cartesian Genetic Programming) - City Relationship Discovery
==================================================================
...
OUTPUT_DIR = r"c:\Users\Utente\Documents\GitHub\gcpaida\cgp_results"
...
def prepare_data(filepath):
    ...
    city_cols = [c for c in df.columns if c.endswith('_infected')]
    city_names = [c.replace('_infected', '') for c in city_cols]
...
def plot_network_graph(connections, city_names, output_dir, top_n=50):
    ...
    ax.set_title('CGP-Discovered City Infection Network\n'
...
def plot_connection_heatmap(connection_matrix, city_names, output_dir):
    ...
    ax.set_title('CGP Connection Strength Heatmap (R² of prediction)', fontsize=14, fontweight='bold')
...
def plot_top_connections(connections, output_dir, top_n=40):
    ...
    ax.set_title(f'Top {top_n} Strongest City Connections (CGP)', fontsize=14, fontweight='bold')
...
def plot_hub_cities(city_connection_count, output_dir):
    ...
    ax.set_title('City Hub Analysis: Number of CGP-Discovered Connections',
...
def plot_cgp_fitness_examples(fitness_histories, output_dir, n_examples=6):
    ...
    for idx, city in enumerate(example_cities):
        ...
        ax.set_yscale('log')
    fig.suptitle('CGP Evolution Fitness Convergence', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_city_influence_detail(target, active_cities, r2_scores, output_dir):
    ...
    ax.set_title(f'Cities Influencing {target.title()} (CGP)', fontsize=13, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='x')
    ax.invert_yaxis()
    plt.tight_layout()
...
def plot_grouped_connections(connections, city_names, output_dir):
    ...
    ax.set_title('Top 30 Most Connected Cities - Connection Matrix (CGP R²)',
...
def main():
    print("=" * 70)
    print("   CGP - Cartesian Genetic Programming")
