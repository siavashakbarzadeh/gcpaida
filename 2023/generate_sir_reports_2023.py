"""
Generate Farsi and English Word document reports for SIR model results (2023).
"""
import os
import pandas as pd
...
SIR_DIR = r"c:\Users\Utente\Documents\GitHub\gcpaida\2023\sir-result-2023"
...
def generate_farsi_report(summary_df):
    ...
    add_styled_table(doc, ['شاخص', 'مقدار'], [
        ['تعداد شهرها', '107'], ['بازه زمانی', '2023-01-01 تا 2023-12-31'],
    ...
    path = os.path.join(OUTPUT_DIR, 'SIR_Report_Farsi_2023.docx')
    doc.save(path)
...
def generate_english_report(summary_df):
    ...
    doc.add_paragraph('SIR model applied to COVID-19 data from 107 Italian provinces for 2023.')
    ...
    add_styled_table(doc, ['Metric', 'Value'], [
        ['Cities', '107'], ['Period', '2023-01-01 to 2023-12-31'], ['Days', '365'],
    ...
    total_cases = summary_df['total_cases'].sum()
    avg_R0 = summary_df['mean_R0'].mean()
    doc.add_paragraph(f'Total 2023 infections: {total_cases:,.0f}. Average R0: {avg_R0:.2f}.')
    ...
    doc.add_paragraph('Beta varied significantly throughout 2023 reflecting pandemic waves and interventions.')
    doc.add_heading('7. Conclusions', level=1)
    for item in [f'Total 2023 infections: {total_cases:,.0f}', f'Average R0: {avg_R0:.2f}',
    ...
    path = os.path.join(OUTPUT_DIR, 'SIR_Report_English_2023.docx')
    doc.save(path)
...
def main():
    ...
    print("Generating SIR reports for 2023...")
