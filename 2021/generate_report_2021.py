"""
Generate a Word document report (Farsi) for CGP city analysis results (2021).
"""
import os
import pandas as pd
...
CGP_DIR = r"c:\Users\Utente\Documents\GitHub\gcpaida\2021\cgp_results"
OUTPUT_DIR = r"c:\Users\Utente\Documents\GitHub\gcpaida\2021\reports"
...
def main():
    ...
    intro2 = (
        'این تحلیل بر روی داده‌های روزانه ۱۰۷ استان (Province) ایتالیا در سال ۲۰۲۱ انجام شده است. '
    ...
    data_desc = (
        ...
        'برای ۱۰۷ استان ایتالیا در سال ۲۰۲۱ (۳۶۵ روز) است. '
        'این داده‌ها از فایل‌های CSV هر شهر استخراج و در یک فایل واحد (merged_cities_2021.csv) ادغام شده‌اند.'
    )
    ...
    param_desc = 'در جدول زیر پارامترهای استفاده‌شده برای الگوریتم CGP بیان شده‌اند:'
    ...
    if os.path.exists(img_path):
        doc.add_paragraph('شکل ۳: گراف شبکه ارتباطات CGP بین شهرهای ایتالیا (۲۰۲۱)').alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    analysis_sections = [
        ('الگوی جغرافیایی ارتباطات',
         'یافته‌های CGP نشان می‌دهد که ارتباطات مستقیم بین شهرها عمدتاً '
         'تابع نزدیکی جغرافیایی است. شهرهای هم‌جوار در یک منطقه (Region) '
         'بیشترین ارتباط را با یکدیگر دارند. '
        ...
        ('خوشه‌های منطقه‌ای',
         ...
         'این خوشه‌ها کاملاً با تقسیمات جغرافیایی و اداری ایتالیا منطبق هستند.'),
    ...
    conclusion = (
        ...
        'ارتباطات مستقیم بین ۱۰۷ استان ایتالیا از نظر انتشار COVID-19 در سال ۲۰۲۱ بررسی شد. '
    ...
    conclusions = [
        ...
        'قوی‌ترین ارتباطات بین شهرهای هم‌جوار جغرافیایی مشاهده شد',
        'خوشه‌های منطقه‌ای واضحی شناسایی شد که با تقسیمات جغرافیایی ایتالیا منطبق هستند',
        'CGP توانست روابط غیرخطی و تأخیرهای زمانی بین شهرها را با موفقیت مدل‌سازی کند',
        'این یافته‌ها می‌تواند در برنامه‌ریزی سیاست‌های بهداشتی منطقه‌ای مورد استفاده قرار گیرد',
    ]
    ...
    output_path = os.path.join(OUTPUT_DIR, 'CGP_Report_Farsi_2021.docx')
